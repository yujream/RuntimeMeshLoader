...
void FindMeshInfo(const aiScene* scene, aiNode* node, FReturnedData& result)
{
	...
	for (uint32 i = 0; i < node->mNumMeshes; i++)
	{
		...
		FModelMeshInfo &mi = result.meshInfo[meshidx];
...
UImportMeshHelperAsync* UImportMeshHelperAsync::LoadMeshAsync(const FString& FilePath, EPathType Type)
{
	...
	Async(EAsyncExecution::Thread, [BlueprintNode, FilePath, Type]()
	{
		FReturnedData Result = BlueprintNode->LoadMesh(FilePath, Type);

		AsyncTask(ENamedThreads::GameThread, [BlueprintNode, Result]()
		{
			// Broadcast in GameThread
			BlueprintNode->OnComplete.Broadcast(Result);
		});
	});
	return BlueprintNode;
}

FReturnedData UImportMeshHelperAsync::LoadMesh(FString filepath, EPathType type)
{
